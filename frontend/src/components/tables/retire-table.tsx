import type {
  RetireVulnerability,
  RetireVulnerabilitySeverity,
} from "@/types/types";
import { withStrictMode } from "@/utils/withStrictMode";

interface RetireTableProps {
  vulnerabilities: RetireVulnerability[];
}

const severityStyles: Record<RetireVulnerabilitySeverity, string> = {
  critical: "bg-red-500/10 text-red-400 border-red-500/20",
  high: "bg-orange-500/10 text-orange-400 border-orange-500/20",
  medium: "bg-amber-500/10 text-amber-400 border-amber-500/20",
  low: "bg-stone-500/10 text-stone-400 border-stone-500/20",
};

// TODO: implement sorting by severity header
// TODO: implement clicking on CVE value on row leading to a link (the link's in the json already)
function RetireTable({ vulnerabilities }: RetireTableProps) {
  return (
    <>
      <div className="mt-8 w-full overflow-hidden border-2 border-stone-800 bg-stone-900 shadow-xl backdrop-blur-sm">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-stone-300">
            {/* Table Header */}
            <thead className="border-b border-accent bg-linear-to-br from-orange-400 to-accent text-xs font-bold tracking-wide text-stone-50 uppercase">
              <tr>
                <th scope="col" className="px-6 py-4">
                  Vulnerability ID
                </th>
                <th scope="col" className="px-6 py-4">
                  Severity
                </th>
                <th scope="col" className="px-6 py-4">
                  CWE(s)
                </th>
                <th scope="col" className="px-6 py-4">
                  Description
                </th>
              </tr>
            </thead>

            {/* Table Body */}
            <tbody className="divide-y-2 divide-stone-900 bg-stone-950">
              {vulnerabilities.map((vuln, index) => {
                // Extract severity, defaulting to "low" if missing
                const severity = vuln.ratings?.[0]?.severity || "low";

                return (
                  <tr
                    key={vuln.id || index}
                    className="transition-colors hover:bg-stone-900/40"
                  >
                    {/* ID */}
                    <td className="px-6 py-4 font-medium whitespace-nowrap text-stone-100">
                      {vuln.id}
                    </td>

                    {/* RetireVulnerabilitySeverity Badge */}
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span
                        className={`inline-flex items-center rounded-md border px-2.5 py-0.5 text-xs font-semibold uppercase ${severityStyles[severity]}`}
                      >
                        {severity}
                      </span>
                    </td>

                    {/* CWE Numbers */}
                    <td className="px-6 py-4 font-mono whitespace-nowrap text-stone-400">
                      {vuln.cwes && vuln.cwes.length > 0
                        ? vuln.cwes.map((cwe) => `CWE-${cwe}`).join(", ")
                        : "--"}
                    </td>

                    {/* Description */}
                    <td className="max-w-xl min-w-75 px-6 py-4 leading-relaxed text-stone-400">
                      {vuln.description}
                    </td>
                  </tr>
                );
              })}

              {/* Empty State Fallback */}
              {vulnerabilities.length === 0 && (
                <tr>
                  <td
                    colSpan={4}
                    className="px-6 py-12 text-center text-stone-500"
                  >
                    No vulnerabilities detected. Your slop is clean.
                  </td>
                </tr>
              )}
            </tbody>
          </table>
        </div>
      </div>
    </>
  );
}

export default withStrictMode(RetireTable);
